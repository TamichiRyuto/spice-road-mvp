...
import { CurryShop, User, UserRegistration } from './types';
...
const loadShopsData = (): CurryShop[] => {
  try {
    // Try Docker path first, then local development path
    let dataPath = path.join(__dirname, '../../database/shops.json');
    if (!fs.existsSync(dataPath)) {
      dataPath = path.join(__dirname, '../database/shops.json');
    }
    if (!fs.existsSync(dataPath)) {
      dataPath = path.join(process.cwd(), 'database/shops.json');
    }
...
const loadUsersData = (): User[] => {
  try {
    // Try Docker path first, then local development path
    let dataPath = path.join(__dirname, '../../database/users.json');
    if (!fs.existsSync(dataPath)) {
      dataPath = path.join(__dirname, '../database/users.json');
    }
    if (!fs.existsSync(dataPath)) {
      dataPath = path.join(process.cwd(), 'database/users.json');
    }
...
const saveUsersData = (users: User[]): boolean => {
  try {
    // Try Docker path first, then local development path
    let dataPath = path.join(__dirname, '../../database/users.json');
...
const calculateMatchScore = (userPrefs: CurryShop['spiceParameters'], shopSpices: CurryShop['spiceParameters']): number => {
